    ...
    public class I2CBase
    {
        ...
        protected void WriteRead(byte[] writeBuffer, byte[] readBuffer)
        {
            try
            {
                if (Device == null)
                {
                    throw new NullReferenceException($"{nameof(I2cDevice)} was null. {nameof(WriteRead)} operation failed.");
                }
                lock (Device)
                {
        ...
        protected void Read(byte[] readBuffer)
        {
            try
            {
                if (Device == null)
                {
                    throw new NullReferenceException($"{nameof(I2cDevice)} was null. {nameof(Read)} operation failed.");
                }
                lock (Device)
                {
        ...
        protected void Write(byte[] writeBuffer)
        {
            try
            {
                if (Device == null)
                {
                    throw new NullReferenceException($"{nameof(I2cDevice)} was null. {nameof(Write)} operation failed.");
                }
                lock (Device)
                {
